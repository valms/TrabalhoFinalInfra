...
SSH_KEY_FILE="${SSH_KEY_FILE:-~/.ssh/unifor-terraform}"
...
{
  printf '[web]\n'
  printf "%s ansible_user=%s ansible_ssh_private_key_file=%s ansible_ssh_common_args='-o StrictHostKeyChecking=no'\n" \
    "${PUBLIC_IP}" \
    "${SSH_USER}" \
    "${SSH_KEY_FILE}"
} > "${INVENTORY_FILE}"
